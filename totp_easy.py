...
def generate_totp_plus(key:str, timestamp:int, window:int=30000, token_long:int=128) -> str:
    ...
    timestamp_bytes = timestamp.encode()
    timestep = int(int(timestamp) / window)
    hash_algorithms = [hashlib.sha256, hashlib.sha384, hashlib.sha512, hashlib.md5, hashlib.sha1, hashlib.sha224, hashlib.sha3_256, hashlib.sha3_384, hashlib.sha3_512, hashlib.sha3_224]
    macs = b""
    for hash_algorithm in hash_algorithms:
        mac = hmac.new(hmac.new(key_bytes, timestep.to_bytes(8, "big"), hash_algorithm).digest(), timestep.to_bytes(8, "little"), hash_algorithm).digest()
        macs += mac
        mac = hmac.new(key_bytes, timestep.to_bytes(8, "big"), hash_algorithm).digest()
        macs += mac
        mac = hmac.new(hmac.new(key_bytes, timestep.to_bytes(8, "little"), hash_algorithm).digest(), timestep.to_bytes(8, "big"), hash_algorithm).digest()
        macs += mac
        mac = hmac.new(key_bytes, timestep.to_bytes(8, "little"), hash_algorithm).digest()
        macs += mac
    encoded_macs = base64.b64encode(macs).rstrip(b"=")
...
def generate_totp_now(p_key:str, p_window:int=30000, p_token_long:int=128) -> str:
    timestamp = str(int(time.time() * 1000))
    return generate_totp(p_key, timestamp, window=p_window, token_long=p_token_long)

def generate_totp_plus_now(p_key:str, p_window:int=30000, p_token_long:int=128) -> str:
    timestamp = str(int(time.time() * 1000))
    return generate_totp_plus(p_key, timestamp, window=p_window, token_long=p_token_long)
...
def generate_totp_easy(key:str, timestamp:int, window:int=30000, token_long:int=128) -> str:
    key_bytes = key.encode()
    timestamp_bytes = timestamp.encode()
    timestep = int(int(timestamp) / window)
...
def generate_totp_easy_now(p_key:str, p_window:int=30000, p_token_long:int=128) -> str:
    timestamp = str(int(time.time() * 1000))
    return generate_totp_easy(p_key, timestamp, window=p_window, token_long=p_token_long)
